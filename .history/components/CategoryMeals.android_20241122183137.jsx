...
const List = ({data}) => {
  ...
  const handleDetails = () => {
    router.navigate({
      pathname: "/mealDetail",
      params: { data: data, title: data.data.title },
    });
  };
    ...
    <TouchableNativeFeedback onPress={handleDetails}>
      <View style={styles.card}>
        <View style={styles.imageContainer}>
          <ImageBackground style={styles.image} source={{ uri: data.data.imageUrl }}>
            <Text style={styles.title}>{data.data.title}</Text>
          </ImageBackground>
        </View>

        <View style={styles.details}>
          <Text style={styles.text}>{data.data.duration + "m"}</Text>
          <Text style={styles.text}>{data.data.complexity.toUpperCase()}</Text>
          <Text style={styles.text}>{data.data.affordability.toUpperCase()}</Text>
        </View>
      </View>
